...
pub async fn perform_stresstest(
    ...
    for task in finished_tasks {
        ...
        println!();
        println!("## Workload {} (concurrency: {})", workload.name.bold().blue(), workload.concurrency.bold());
        ...
        println!("  size avg: {}; p50: {p50:.2}; p90: {p90:.2}; p99: {p99:.2}", avg.bold());
...
fn print_percentiles<T: fmt::Debug>(sketch: &DDSketch, map: impl Fn(f64) -> T) {
    ...
    println!("  avg: {:.2?}; p50: {p50:.2?}; p90: {p90:.2?}; p99: {p99:.2?}", avg.bold());
}
